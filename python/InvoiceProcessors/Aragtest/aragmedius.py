...
st.title("Arag Medius Template Updater")
...
if invoice_file and template_file:
    try:
        ...
        for _, row in df_code_map_div.iterrows():
            ...
            total = filtered_df['Real Premium'].sum()
        ...
        company_totals = df_invoice[['Company', 'Real Premium']].dropna().groupby('Company')['Real Premium'].sum().reset_index()
        ...
        for invoice_company, interco in interco_mapping.items():
            total = company_totals[company_totals['Company'] == invoice_company]['Real Premium'].sum()
            if total > 0:
        ...
        df_cc_totals = df_hhi_thc.groupby('CC_Code')['Real Premium'].sum().reset_index()
        ...
        match_indices = cc_merged_df[cc_merged_df['Real Premium'].notna()].index
        df_template.loc[match_indices, 'NET'] = cc_merged_df.loc[match_indices, 'Real Premium']
