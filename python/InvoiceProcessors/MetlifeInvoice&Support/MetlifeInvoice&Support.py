...
st.title("Metlife Invoice and Support Generator")
...
if invoice_file and template_file:
    ...
    detail_df = pd.read_excel(invoice_xls, sheet_name='Client Detail', engine='openpyxl')
    ...
    def convert_df_to_excel(df1, df2, df3):
        ...
        wb = Workbook()
        ws = wb.active
        ws.title = 'Metlife Invoice and Support'
    ...
    st.download_button(
        label="Download Metlife Invoice and Support Excel",
        data=excel_data,
        file_name="Metlife_Invoice_and_Support.xlsx",
