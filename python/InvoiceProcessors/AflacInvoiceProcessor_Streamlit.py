...
template_file = st.file_uploader("Upload Template Excel File (with Code Map sheet)", type=["xlsx"])
...
if invoice_file and template_file:
    ...
    grand_total = final_df['Sum of Monthly Premium'].sum()
    final_df = pd.concat([final_df, pd.DataFrame([{
        'Row Labels': 'Grand Total',
        'Sum of Monthly Premium': grand_total,
        'Full Company Name': ''
    }])], ignore_index=True)

    # Table 2: Company & Division Breakdown
    ...
    # Table 3: THC & HHI Department Breakdown
